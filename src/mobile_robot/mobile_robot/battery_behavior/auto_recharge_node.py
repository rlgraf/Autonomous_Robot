...
#
# Edits applied:
#   1) Publish /recharge_active=True from low-battery trigger until battery full.
#      (ARRIVED no longer publishes False.)
#   2) Latch /recharge_active publishing to avoid spamming.
#   3) On avoidance pause rising-edge, publish one STOP to /cmd_vel_recharge.
#
...
class AutoRechargeNode(Node):
    ...
    def _paused_cb(self, msg: Bool):
        ...
        # Rising edge: stop once so we don't leave a stale motion command "latched" downstream
...
def _control_loop(self):
    ...
    # ── IDLE: Trigger homing when battery is low ──────────────────────────
    ...
    # ── Global exit: if battery is full, relinquish recharge control ───────
    ...
    # From here on, recharge mode is active
    self._publish_active(True)
    ...
    # ── ARRIVED: hold position and wait for charging ──────────────────────
